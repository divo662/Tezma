...
class KidsMoviesWidget extends StatefulWidget {
  const KidsMoviesWidget({Key? key}) : super(key: key);

  @override
  State<KidsMoviesWidget> createState() => _KidsMoviesWidgetState();
}

class _KidsMoviesWidgetState extends State<KidsMoviesWidget> {
  ...
  Future<List<MovieModel>> fetchKidsMovies1({int page = 1, String? query}) async {
    const apiKey = '9ee736e148e808222f04c1535dc80b64';

    // Fetch kids movies if query is not provided
    if (query == null || query.isEmpty) {
      final url =
          'https://api.themoviedb.org/3/discover/movie?api_key=$apiKey&sort_by=popularity.desc&certification_country=US&certification.lte=G&with_genres=16&include_adult=false';
    ...
    final url =
        'https://api.themoviedb.org/3/search/movie?api_key=$apiKey&query=$encodedQuery&page=1&language=en-US';
...
// Set an empty list to reset the movies
          ...
          Expanded(
            child: FutureBuilder<List<MovieModel>>(
              future: fetchKidsMovies1(page: currentPage, query: _searchQuery),
