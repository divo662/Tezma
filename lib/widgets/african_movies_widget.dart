...
class AfricanMoviesWidget extends StatefulWidget {
  const AfricanMoviesWidget({Key? key}) : super(key: key);

  @override
  State<AfricanMoviesWidget> createState() => _AfricanMoviesWidgetState();
}

class _AfricanMoviesWidgetState extends State<AfricanMoviesWidget> {
  ...
  Future<List<MovieModel>> fetchAfricanMovies1({int page = 1, String? query}) async {
    const apiKey = '9ee736e148e808222f04c1535dc80b64';

    // Fetch African movies if query is not provided
    if (query == null || query.isEmpty) {
      final url =
          'https://api.themoviedb.org/3/discover/movie?api_key=$apiKey&sort_by=popularity.desc&with_original_language=sw';
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title: const Text("African Movie"),
...
// Set an empty list to reset the movies
          ...
          Expanded(
            child: FutureBuilder<List<MovieModel>>(
              future: fetchAfricanMovies1(page: currentPage, query: _searchQuery),
