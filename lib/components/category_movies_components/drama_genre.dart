...
class DramaGenre extends StatefulWidget {
  const DramaGenre({Key? key}) : super(key: key);

  @override
  State<DramaGenre> createState() => _DramaGenreState();
}

class _DramaGenreState extends State<DramaGenre> {
  ...
  void initState() {
    ...
    fetchDramaGenre(page: 1).then((movies) {
      setState(() {
        _movies = movies;
      });
    }).catchError((error) {
        ...
        children: [
          Expanded(
            child: FutureBuilder<List<MovieModel>>(
              future: fetchDramaGenre(page: currentPage, query: _searchQuery),
